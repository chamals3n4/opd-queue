...
public class SlipGeneratorService {
    ...
    private byte[] buildPdf(QueueTicket ticket) throws Exception {
        ...
        addRow(table, "Est. Wait", ticket.getEstimatedWaitMinutes() + " min", bold, regular);
        addRow(table, "Date", ticket.getIssuedAt()
                .format(DateTimeFormatter.ofPattern("dd MMM yyyy")), bold, regular);
        addRow(table, "Time", ticket.getIssuedAt()
                .format(DateTimeFormatter.ofPattern("hh:mm a")), bold, regular);
